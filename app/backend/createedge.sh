...
if [ ! -d "cloudinit/user-data" ]; then
    mkdir backend/cloudinit/user-data
fi

touch backend/cloudinit/user-data/$1
cat --show-tabs backend/clondinit/user-data | sed "s/{{hostname}}/$1/g" > backend/cloudinit/user-data/$1
